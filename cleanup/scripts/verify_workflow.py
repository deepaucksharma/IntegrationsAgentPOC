"""
Standalone verification workflow script.
"""
import os
import sys
import asyncio
...
async def render_and_execute_template():
    ...
    try:
        ...
        # Load infra_agent_verify.ps1.j2 template
        template = env.get_template('infra_agent_verify.ps1.j2')
        ...
        # Render the template
        logger.info("Rendering verification template...")
        ...
        with tempfile.NamedTemporaryFile(suffix=".ps1", delete=False, mode='w') as temp:
            temp_path = temp.name
            temp.write(rendered)
            logger.info(f"Rendered verification script written to: {temp_path}")
        
        # Execute the script
        logger.info("Executing verification script...")
        ...
        logger.info("Verification script execution complete.")
...
if __name__ == "__main__":
    ...
    logger.info("Starting verification workflow execution...")
