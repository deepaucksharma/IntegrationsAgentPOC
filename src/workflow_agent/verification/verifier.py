...
class Verifier:
    ...
    async def verify_result(
        ...
        if state.action == "install":
            if "installed" not in stdout and "complete" not in stdout and "success" not in stdout:
                return {"warning": "Verification warning: Installation success indicators not found in output"}
        elif state.action == "remove":
            if "removed" not in stdout and "uninstalled" not in stdout and "success" not in stdout:
